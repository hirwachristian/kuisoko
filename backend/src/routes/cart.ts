  ...
  try {
    const result = await pool.query(
      `SELECT product_id AS "productId", quantity, selected_color AS "selectedColor",
              selected_size AS "selectedSize", unit_price AS "unitPrice"
...
router.put('/:productId', authenticate, async (req, res, next) => {
  try {
    ...
    await pool.query(
      `INSERT INTO cart_items (user_id, product_id, quantity, selected_color, selected_size, unit_price)
       VALUES ($1, $2, $3, $4, $5, $6)
       ON CONFLICT (user_id, product_id) DO UPDATE SET
         ...
         updated_at = now()`,
      [req.authUser!.id, req.params.productId, body.quantity, body.selectedColor ?? null, body.selectedSize ?? null, body.unitPrice ?? null]
    );
