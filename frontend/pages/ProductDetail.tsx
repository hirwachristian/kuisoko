    ...
    <div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 py-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-20">
        <div className="space-y-4">
          <div className="relative aspect-square rounded-[2rem] overflow-hidden bg-white border border-slate-100 shadow-xl shadow-slate-200/50">
          ...
          <div className="flex gap-4 flex-wrap">
            {displayImages.map((img, i) => (
              <button
                ...
                className={`w-24 h-24 rounded-2xl overflow-hidden bg-white border-2 transition-all ${activeVideoIndex === null && activeImgIndex === i ? 'border-emerald-600 shadow-lg' : 'border-slate-100 opacity-60'}`}
                aria-label={t('detail_view_image', { n: i + 1 })}
              >
                <img src={img} alt={`${product.name} thumbnail ${i + 1}`} className="w-full h-full object-contain p-1.5" />
              </button>
            ))}
            {displayVideos.map((videoUrl, i) => (
              <button
                ...
                className={`relative w-24 h-24 rounded-2xl overflow-hidden bg-slate-900 border-2 transition-all flex items-center justify-center ${activeVideoIndex === i ? 'border-emerald-600 shadow-lg' : 'border-slate-100 opacity-60'}`}
                aria-label={t('detail_view_video', { n: i + 1 })}
              >
                <video src={videoUrl} className="w-full h-full object-cover opacity-70" muted />
                <Play size={28} className="absolute text-white fill-white drop-shadow" />
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-col justify-center">
          ...
          <div className="flex items-baseline gap-3 mb-6">
            ...
            {isOutOfStock ? (
              <span className="text-sm font-bold text-rose-600">{t('product_out_of_stock')}</span>
            ) : stockLevel === 'low' ? (
              <span className="text-sm font-bold text-rose-500">{t('product_only_left', { n: product.stock })}</span>
            ) : stockLevel === 'medium' ? (
              <span className="text-sm font-bold text-orange-500">{t('product_in_stock', { n: product.stock })}</span>
            ) : (
              <span className="text-sm font-bold text-emerald-600">{t('product_in_stock', { n: product.stock })}</span>
            )}
          </div>
          ...
          <div className="space-y-6 mb-10">
            <div className="flex items-center gap-6">
              <div className="flex items-center border-2 border-slate-100 rounded-2xl p-1">
                ...
                <span className="w-12 text-center font-bold text-lg text-slate-900 dark:text-emerald-100">{qty}</span>
                <button
                   onClick={() => setQty(q => Math.min(product.stock, q + 1))}
                   disabled={isOutOfStock || qty >= product.stock}
                   className="w-10 h-10 flex items-center justify-center text-slate-400 hover:text-emerald-600 hover:bg-slate-50 rounded-xl transition-all disabled:opacity-40 disabled:hover:bg-transparent disabled:cursor-not-allowed"
                >
                  <Plus size={20} />
                </button>
              </div>
              <div className="flex-1 flex gap-4">
                <button
                  ...
                  className="flex-1 bg-emerald-600 dark:bg-emerald-600 hover:bg-emerald-700 dark:hover:bg-emerald-700 text-white h-14 rounded-2xl font-bold flex items-center justify-center gap-3 shadow-lg transition-all active:scale-95 disabled:opacity-50 disabled:hover:bg-emerald-600 disabled:cursor-not-allowed"
                >
                  <ShoppingCart size={20} /> {t('product_add_to_cart')}
                </button>
                <button
                  onClick={handleBuyNow}
                  disabled={isOutOfStock}
                  className="flex-1 bg-orange-500 dark:bg-orange-500 hover:bg-orange-600 dark:hover:bg-orange-600 text-white h-14 rounded-2xl font-bold flex items-center justify-center gap-3 shadow-lg transition-all active:scale-95 disabled:opacity-50 disabled:hover:bg-orange-500 disabled:cursor-not-allowed"
                >
                  {isOutOfStock ? t('product_sold_out') : t('product_buy_now')}
                </button>
              </div>
            </div>
          </div>
      ...
      <div className="bg-slate-50 dark:bg-slate-800 p-1 rounded-2xl flex gap-1 mb-8">
        <button
      ...
      {activeTab === 'description' ? (
        ...
        <div className="space-y-8">
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white">{t('detail_customer_reviews')}</h2>

          {reviewsList.length > 0 ? (
            <div className="space-y-6">
              {reviewsList.map(review => (
                <div key={review.id} className={`bg-white dark:bg-slate-800 p-6 rounded-2xl border shadow-sm ${review.isHidden ? 'border-rose-200 dark:border-rose-900 opacity-70' : 'border-slate-100 dark:border-slate-700'}`}>
                  <div className="flex items-start gap-4">
                    <span className="w-10 h-10 rounded-full bg-emerald-700 dark:bg-emerald-800 flex items-center justify-center text-white text-sm font-bold flex-shrink-0">
                    ...
                    <div className="flex-1">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <div className="flex items-center gap-2">
                          ...
                          {review.isHidden && (
                            <span className="inline-flex items-center rounded-full bg-rose-100 dark:bg-rose-900/40 px-2.5 py-0.5 text-xs font-bold text-rose-700 dark:text-rose-300">{t('detail_hidden')}</span>
                          )}
                        </div>
                      ...
                      <div className="flex gap-0.5 my-1">
                        {[1, 2, 3, 4, 5].map(star => (
                          <Star key={star} size={16} className={star <= review.rating ? 'text-amber-400' : 'text-slate-200 dark:text-slate-700'} fill={star <= review.rating ? 'currentColor' : 'none'} />
                        ))}
                      </div>
                      <p className="text-slate-600 dark:text-slate-300 leading-relaxed">{review.comment}</p>
                      {review.image && (
                        <img src={review.image} alt="Review attachment" className="mt-3 w-24 h-24 rounded-xl object-cover border border-slate-100 dark:border-slate-700" />
                      )}
                    </div>
                  </div>
                </div>
            ...
            <p className="text-slate-500 dark:text-slate-400">{t('detail_no_reviews_yet')}</p>
          )}
      ...
      {similarProducts.length > 0 && (
        <div className="mt-20">
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-8">{t('detail_similar_items')}</h2>
