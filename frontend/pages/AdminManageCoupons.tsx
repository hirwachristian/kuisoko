...
const AdminManageCoupons: React.FC = () => {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-8">
      <div>
        <h2 className="text-3xl font-black text-slate-900 dark:text-emerald-50 mb-2">Coupons</h2>
        <p className="text-slate-600 dark:text-emerald-300 text-sm max-w-xl">Create and manage promo codes customers can apply at checkout.</p>
      </div>

      <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-5 sm:p-8 border border-slate-100 dark:border-slate-800 shadow-sm">
        <h3 className="text-xl font-bold text-slate-900 dark:text-emerald-50 mb-6 flex items-center gap-2">
          <Ticket size={20} className="text-orange-500" /> New Coupon
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          <div>
            <label className="block text-sm font-bold text-slate-700 dark:text-emerald-300 mb-2">Code</label>
            <input
              type="text"
              value={code}
          ...
          <div>
            <label className="block text-sm font-bold text-slate-700 dark:text-emerald-300 mb-2">Discount type</label>
            <select
              value={discountType}
          ...
          <div>
            <label className="block text-sm font-bold text-slate-700 dark:text-emerald-300 mb-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div>
            <label className="block text-sm font-bold text-slate-700 dark:text-emerald-300 mb-2">Min. order (RWF, optional)</label>
            <input
              type="number"
          ...
          <div>
            <label className="block text-sm font-bold text-slate-700 dark:text-emerald-300 mb-2">Usage limit (optional)</label>
            <input
              type="number"
              value={usageLimit}
          ...
          <div>
            <label className="block text-sm font-bold text-slate-700 dark:text-emerald-300 mb-2">Expires (optional)</label>
            <input
              type="date"
              value={expiresAt}
        ...
        {formError && <p className="text-rose-600 dark:text-rose-400 text-sm mb-4">{formError}</p>}
        <button
          ...
          <table className="min-w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-950 border-b border-slate-100 dark:border-slate-800">
                ...
                <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Min. Order</th>
                <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Usage</th>
                <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Expires</th>
                <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Active</th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {isLoading ? (
                <tr><td colSpan={7} className="text-center py-10 text-slate-500 dark:text-emerald-300">Loading...</td></tr>
              ) : coupons.length === 0 ? (
                <tr><td colSpan={7} className="text-center py-10 text-slate-500 dark:text-emerald-300">No coupons yet.</td></tr>
              ) : (
                coupons.map((coupon) => (
                  <tr key={coupon.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-950 transition-colors">
                    <td className="px-6 py-4 font-bold text-slate-900 dark:text-emerald-50">{coupon.code}</td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 dark:text-emerald-200">
